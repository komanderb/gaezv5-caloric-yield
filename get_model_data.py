from src.utils import (sum_groups_kcal, get_crop_mapping, get_cal_mapper,
                       gaezv5_path, preload_har_cache, check_urls_exist)
...
def _build_yield_urls(yield_var, crop_mapping, calorie_mapping):
    """Enumerate all yield URLs that will be requested during the run."""
    urls = set()
    groups = [g for g in calorie_mapping if g in crop_mapping]
    for (period, model, scen) in valid_combos():
        for water in WATERS:
            for group in groups:
                for crop in crop_mapping[group]:
                    urls.add(gaezv5_path(
                        variable_code=yield_var,
                        period=period, climate_model=model, scenario=scen,
                        crop=crop, water_code=water,
                    ))
    return urls
...
def run():
    ...
    for yield_var in YIELD_VARS:
        ...
        yield_urls = _build_yield_urls(yield_var, crop_mapping, cal_map_t6)
